                          ...
                          uint32_t bytesInFlight)
{
  ...
  m_lastTime = tcb->m_timestampToEcho;
...
TcpCubic::measure_delay (Ptr<TcpSocketState> tcb)
{
  ...
  // Calculate delay
  uint32_t delay = (tcp_time_stamp() - tcb->m_timestampToEcho) << 3;

  NS_LOG_DEBUG("Calculate delay = (new_timestamp - echo-from-ack) << 3\n  "
    << delay << " = ( " << tcp_time_stamp() << " - " << tcb->m_timestampToEcho
    << ") << 3" );
  ...
  m_previousTimeStamp = tcb->m_timestampToEcho;
  ...
  if ( m_lastCwnd == cwndSeg &&
       (int32_t)(tcp_time_stamp() - m_lastTime) <= HZ / 32)
    {
      NS_LOG_DEBUG("  m_lastCwnd = cwndSeg = " << cwndSeg
        ...
        << tcb->m_timestampToEcho << " - " << m_lastTime
        << ") <= " << HZ << " / 32" );
      return;
    }
  else
